pipeline {
  ...
  stages {
    ...
    stage('Trivy Filesystem Scan') {
      steps { sh 'trivy fs --severity HIGH,CRITICAL --exit-code 1 --no-progress .' }
    }
    stage('Build Image') { steps { sh 'docker build --pull -t $IMAGE .' } }
    stage('Trivy Image Scan') {
      steps { sh 'trivy image --severity HIGH,CRITICAL --exit-code 1 --no-progress $IMAGE' }
    }
    stage('Push Image') {
    ...
    stage('Deploy to GKE') {
      ...
      steps { withCredentials([file(credentialsId: 'gke-kubeconfig', variable: 'KUBECONFIG_FILE')]) {
  ...
  post {
    always { junit allowEmptyResults: true, testResults: 'test-results.xml'; cleanWs() }
    success { echo "Deployment succeeded: ${IMAGE}" }
    failure { echo 'Pipeline failed. Check the stage logs and scan reports.' }
  }
}
